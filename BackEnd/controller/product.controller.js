...
const RESERVED_QUERY_PARAMS = new Set([
  "category_id", "category_name", "page", "limit",
  "minPrice", "maxPrice", "sort", "is_active",
]);

export async function GetProducts(req, res) {
  try {
    const {
      category_id, category_name, page = 1, limit = 10,
      minPrice, maxPrice, sort, is_active, ...rest
    } = req.query;
    ...
    const matchStage = {};
    matchStage.is_active = is_active !== undefined ? is_active === "true" : true;
    ...
    if (category_name) {
      ...
      if (!category) {
        return res.json({ currentPage: pageNum, totalPages: 0, totalproduct: 0, data: [], message: "No category found with this name" });
      }
    ...
    const attributeKeys = Object.keys(rest).filter(
      (key) => !RESERVED_QUERY_PARAMS.has(key) && rest[key] !== undefined && rest[key] !== "",
    );
    ...
    for (const key of attributeKeys) {
      ...
      const values = String(rest[key]).split(",").map((v) => v.trim()).filter(Boolean);
...
export async function AddBulkProducts(req, res) {
  try {
    ...
    res.status(201).json({ message: "Bulk Products added successfully", data: Products });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
}
...
export async function UpdateProduct(req, res) {
  try {
    ...
    const Products = await productModel.findByIdAndUpdate(productId, UpdatedProduct, { new: true, runValidators: true });
